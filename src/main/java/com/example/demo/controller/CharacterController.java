...
// import com.example.demo.dto.Character; // 캐릭터 모델 클래스 (별도로 생성)
...
public class CharacterController {
    ...
    public void joinGame(SimpMessageHeaderAccessor headerAccessor, @Payload Map<String, String> payload) {
        ...
        String characterId = UUID.randomUUID().toString().substring(0, 5);
        MoveMessage newCharacter = new MoveMessage(sessionId, Math.random() * 580, Math.random() * 480, characterName);
        newCharacter.setName(characterName); 
        connectedCharacters.put(sessionId, newCharacter);

        System.out.println("캐릭터 생성됨: " + newCharacter);

        // 수정: 캐릭터 정보에 characterId를 명시적으로 포함
        Map<String, Object> characterData = Map.of(
            "id", sessionId,
            "x", newCharacter.getX(),
            "y", newCharacter.getY()
        );
